...
def train_model(model,  criterion, triplet, num_epochs):
  ...
  # for the new version like 0.4.0, 0.5.0 and 1.0.0
            ...
            epoch_loss = running_loss / (dataset_sizes[phase] + dataset_sizes1[phase])
            epoch_acc = running_corrects /(dataset_sizes[phase] + dataset_sizes1[phase])
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./train_double.py', dir_name + '/train_double.py')
